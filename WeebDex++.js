// ==UserScript==
// @name         WeebDex++ Optimized
// @namespace    https://weebdex.org/
// @version      1.0.3
// @description  Enhanced QOL for WeebDex - Advanced tracking, filtering, blocking, and more. Optimized performance with fixed API calls and improved UI.
// @author       WeebDex++
...
(function() {
    ...
    if (href.startsWith("https://weebdex.org/title/")) return FORMAT_DETAIL;
    if (href.startsWith("https://weebdex.org/group")) return FORMAT_THUMBNAIL;
    if (href.startsWith("https://weebdex.org/author")) return FORMAT_THUMBNAIL;
    if (href.startsWith("https://weebdex.org/tag")) return FORMAT_THUMBNAIL;
    if (href.startsWith("https://weebdex.org/updates")) return FORMAT_LIST;
    if (href.startsWith("https://weebdex.org/search?sort=createdAt")) return FORMAT_THUMBNAIL; // Changed from FORMAT_LIST
    if (href.startsWith("https://weebdex.org/search")) return FORMAT_THUMBNAIL;
    ...
    function addControllers() {
        ...
        const button1 = createControlButton(hideRead ? "Read Hidden" : "Read Shown", hideRead ? READ_BUTTON_COLOR : "transparent", () => {
            hideEntries();
            hideRead = !hideRead;
            button1.style.backgroundColor = hideRead ? READ_BUTTON_COLOR : "transparent";
            button1.textContent = hideRead ? "Read Hidden" : "Read Shown";
        ...
        const button2 = createControlButton(hideIgnore ? "Ignore Hidden" : "Ignore Shown", hideIgnore ? IGNORE_BUTTON_COLOR : "transparent", () => {
            hideEntries();
            hideIgnore = !hideIgnore;
            button2.style.backgroundColor = hideIgnore ? IGNORE_BUTTON_COLOR : "transparent";
            button2.textContent = hideIgnore ? "Ignore Hidden" : "Ignore Shown";
            categorize(getFormat(window.location.href), window.location.href === CATEGORY_UPDATES);
            hideAllReadFunc();
            document.body.classList.remove('weebdex-hiding');
        });

        const button3 = createControlButton(hideUnmarked ? "Unmarked Hidden" : "Unmarked Shown", hideUnmarked ? UNMARKED_BUTTON_COLOR : "transparent", () => {
            hideEntries();
            hideUnmarked = !hideUnmarked;
            button3.style.backgroundColor = hideUnmarked ? UNMARKED_BUTTON_COLOR : "transparent";
            button3.textContent = hideUnmarked ? "Unmarked Hidden" : "Unmarked Shown";
            categorize(getFormat(window.location.href), window.location.href === CATEGORY_UPDATES);
            hideAllReadFunc();
            document.body.classList.remove('weebdex-hiding');
        });

        const button4 = createControlButton(hideAllRead ? "All Read Hidden" : "All Read Shown", hideAllRead ? HIDE_ALL_READ_BUTTON_COLOR : "transparent", () => {
            hideEntries();
            hideAllRead = !hideAllRead;
            button4.style.backgroundColor = hideAllRead ? HIDE_ALL_READ_BUTTON_COLOR : "transparent";
            button4.textContent = hideAllRead ? "All Read Hidden" : "All Read Shown";
            hideAllReadFunc();
            document.body.classList.remove('weebdex-hiding');
        });
    ...
    function createControlButton(text, bgColor, onClick) {
        ...
        btn.addEventListener("mouseenter", function() {
            this.style.backgroundColor = bgColor === "transparent" ? "#f3f4f6" : this.style.backgroundColor;
    ...
    function getStatistics() {
        ...
        for (let i = 0; i < localStorage.length; i++) {
            ...
            // Skip configuration and cache keys
            if (key.startsWith("_conf_") || key.includes("-")) continue;
    ...
    function startHideObserver() {
        const observer = new MutationObserver(() => {
            ...
            observerTimer = setTimeout(() => {
                const format = getFormat(location.href);
                categorize(format, location.href === CATEGORY_UPDATES);
    ...
    // ==================== SECTION 11: QUEUE SYSTEM ====================
    function handleQueue() {
        if (queue.length > 0) {
            checkPage(queue.shift());
        }
    ...
    function checkPage(entryID) {
        ...
        if (!bypassCache) {
            getMangaFromCache(entryID).then(cached => {
                ...
                fetchFromAPI(entryID, handleData);
            }).catch(() => {
                fetchFromAPI(entryID, handleData);
            });
            return;
    ...
    function fetchFromAPI(entryID, handleData) {
        GM_xmlhttpRequest({
            method: 'GET',
            ...
            onload: function(res) {
                if (res.status >= 200 && res.status < 300) {
                    ...
                    console.log('Rate limited, re-queuing manga ' + entryID);
                    setTimeout(() => {
                        if (!queuedIDs.has(entryID)) {
                            queue.push(entryID);
                            queuedIDs.add(entryID);
                        }
                    }, 2000);
                } else {
                    console.error('Failed to fetch manga ' + entryID + ', status:', res.status);
                }
            },
            onerror: function() {
                console.error('Network error fetching manga ' + entryID);
            }
        });
    }
    ...
    // ==================== SECTION 12: BLOCK USERS ====================
    function blockUsers(format) {
        if (format !== FORMAT_LIST && format !== FORMAT_THUMBNAIL) return;

        const selectors = [
            'article.flex.gap-2.border-t-2.py-2',
        ...
        entries.forEach(entry => {
            ...
            if (!shouldHide) {
                entry.querySelectorAll('a[href*="/group/"]').forEach(link => {
                    const href = link.getAttribute('href');
                    const id = href ? href.split('/').pop() : null;
    ...
    function addButtonsForListFormat() {
        document.querySelectorAll('article.flex.gap-2.border-t-2.py-2').forEach(entry => {
            ...
            if (link && !entry.querySelector('.weebdex-tracker-btns')) {
                ...
                if (entryID) addButtonsForElement(entryID, entry, FORMAT_LIST);
            }
        });
    }
    ...
    function addButtonsForThumbnailFormat() {
        // Try list view first
        const listEntries = document.querySelectorAll('article.flex.gap-2.border-t-2.py-2');
        ...
        // Then try thumbnail view
        const thumbnailSelectors = [
            'article .group.list-card.flex.gap-4',
            '[class*="manga-card"]',
            '.manga-card',
            '.title-card',
            'article[class*="flex"]:not(.flex.gap-2.border-t-2.py-2)' // Exclude already processed list entries
        ];

        const selector = thumbnailSelectors.join(', ');
        document.querySelectorAll(selector).forEach(entry => {
            ...
            if (link && !entry.querySelector('.weebdex-tracker-btns')) {
                const entryID = extractEntryID(link.href);
                if (entryID) addButtonsForElement(entryID, entry, FORMAT_THUMBNAIL);
            }
        });
    }
    ...
    function addButtonsForDetailFormat() {
        ...
        try {
            const entryID = extractEntryID(window.location.href);
            if (entryID) addButtonsForElement(entryID, entry, FORMAT_DETAIL);
        } catch (e) {
    ...
    function addButtonsForElement(entryID, element, format) {
        ...
        if (format === FORMAT_LIST) {
            // List view container
            container = element.querySelector('div.w-0.flex-auto.space-y-1');
        } else {
            // Thumbnail view - find the right container
            container = element.querySelector('.flex.w-0.flex-auto.flex-col');
            if (!container) {
                container = element.querySelector('.manga-info, .card-content') || element;
            }
        }
        ...
        if (format === FORMAT_LIST) {
            // List view placement
            const titleContainer = element.querySelector('h2.truncate.font-semibold');
            if (titleContainer && titleContainer.parentNode) {
                titleContainer.parentNode.insertBefore(btnContainer, titleContainer.nextSibling);
            }
        } else if (format === FORMAT_THUMBNAIL) {
            // Thumbnail view placement - insert after tags div
            const tagsDiv = container.querySelector('.flex.flex-wrap.gap-1');
            if (tagsDiv && tagsDiv.parentNode) {
                tagsDiv.parentNode.insertBefore(btnContainer, tagsDiv.nextSibling);
            } else {
    ...
    function createButtonContainer(entryID, isDetail = false) {
        ...
        btnContainer.style.cssText = isDetail ?
    ...
    function queueEntry(event) {
        ...
        updateButtonColors(event.currentTarget, READ_BUTTON_COLOR, "transparent");
        categorize(getFormat(window.location.href), window.location.href === CATEGORY_UPDATES);
    }
    ...
    function ignoreEntry(event) {
        const entryID = event.currentTarget.getAttribute("entryid");
        localStorage.setItem(entryID, "-1");
        updateButtonColors(event.currentTarget, IGNORE_BUTTON_COLOR, "transparent", true);
        categorize(getFormat(window.location.href), window.location.href === CATEGORY_UPDATES);
    }
    ...
    function updateButtonColors(clickedBtn, clickedColor, otherColor, isIgnore = false) {
...
// ==================== SECTION 15: SETTINGS PANEL ====================
    function toggleSettings() {
        ...
        if (!settingsPanel) {
            ...
            settingsPanel.id = "weebdex-settings";
            ...
            settingsTitle.textContent = "WeebDex++ Settings";
            ...
            userLabel.textContent = "Blocked Users:";
            userLabel.style.cssText = "display:block;font-size:12px;color:#6b7280;margin-bottom:4px;";
            userDiv.appendChild(userLabel);

            const userInput = document.createElement("input");
            userInput.placeholder = "Add user ID to block";
            userInput.style.cssText = "width:calc(100% - 60px);padding:8px;border:1px solid #d1d5db;border-radius:4px;font-size:12px;margin-bottom:8px;";

            const addUserBtn = document.createElement("button");
            addUserBtn.textContent = "Add";
            addUserBtn.style.cssText = "padding:8px;background:#10b981;color:white;border:none;border-radius:4px;cursor:pointer;font-size:12px;margin-left:8px;";
            addUserBtn.addEventListener("click", () => {
                const user = userInput.value.trim();
                if (user && !USER_LIST.includes(user)) {
                    USER_LIST.push(user);
                    userInput.value = "";
            ...
            userInputContainer.style.cssText = "display:flex;margin-bottom:8px;";
            userInputContainer.appendChild(userInput);
            userInputContainer.appendChild(addUserBtn);
            userDiv.appendChild(userInputContainer);

            const userListDiv = document.createElement("div");
            userListDiv.style.cssText = "max-height:100px;overflow-y:auto;border:1px solid #d1d5db;padding:8px;border-radius:4px;font-size:12px;";
            userDiv.appendChild(userListDiv);
            updateBlockedList(userListDiv, USER_LIST, "user");
            ...
            groupLabel.textContent = "Blocked Groups:";
            groupLabel.style.cssText = "display:block;font-size:12px;color:#6b7280;margin-bottom:4px;";
            groupDiv.appendChild(groupLabel);

            const groupInput = document.createElement("input");
            groupInput.placeholder = "Add group ID to block";
            groupInput.style.cssText = "width:calc(100% - 60px);padding:8px;border:1px solid #d1d5db;border-radius:4px;font-size:12px;margin-bottom:8px;";
            const addGroupBtn = document.createElement("button");
            addGroupBtn.textContent = "Add";
            addGroupBtn.style.cssText = "padding:8px;background:#10b981;color:white;border:none;border-radius:4px;cursor:pointer;font-size:12px;margin-left:8px;";
            addGroupBtn.addEventListener("click", () => {
                const group = groupInput.value.trim();
                if (group && !GROUP_LIST.includes(group)) {
            ...
            const groupInputContainer = document.createElement("div");
            groupInputContainer.style.cssText = "display:flex;margin-bottom:8px;";
            groupInputContainer.appendChild(groupInput);
            groupInputContainer.appendChild(addGroupBtn);
            groupDiv.appendChild(groupInputContainer);

            const groupListDiv = document.createElement("div");
            groupListDiv.style.cssText = "max-height:100px;overflow-y:auto;border:1px solid #d1d5db;padding:8px;border-radius:4px;font-size:12px;";
            groupDiv.appendChild(groupListDiv);
            updateBlockedList(groupListDiv, GROUP_LIST, "group");
            ...
            tagLabel.textContent = "Blocked Tags:";
            tagLabel.style.cssText = "display:block;font-size:12px;color:#6b7280;margin-bottom:4px;";
            tagDiv.appendChild(tagLabel);

            const tagContainer = document.createElement("div");
            tagContainer.style.cssText = "max-height:200px;overflow-y:auto;border:1px solid #d1d5db;padding:8px;border-radius:4px;font-size:12px;";
    ...
    function saveConfiguration(users, groups, tags) {
        USER_LIST.length = 0;
        USER_LIST.push(...users.split(",").map(u => u.trim()).filter(u => u));
        GROUP_LIST.length = 0;
        GROUP_LIST.push(...groups.split(",").map(g => g.trim()).filter(g => g));
        ...
        if (Array.isArray(tags)) {
            ...
            TAG_LIST.length = 0;
            TAG_LIST.push(...tags.split(",").map(t => t.trim().toLowerCase()).filter(t => t));
        }
        ...
        forceRecheckNewEntry = true;
        settingsOpen = false;
        const panel = document.querySelector("#weebdex-settings");
        if (panel) panel.remove();
        categorize(getFormat(window.location.href), window.location.href === CATEGORY_UPDATES);
    }

    // ==================== SECTION 16: BLOCK BUTTONS ====================
    ...
    function main() {
        ...
        if (lastTagList !== currentTagList) {
            forceRecheckNewEntry = true;
            setConfig("_conf_tags", currentTagList);
        }
